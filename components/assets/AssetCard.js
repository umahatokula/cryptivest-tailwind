...
function AssetCard({asset, withdraw}) {
  return (
    <div className='flex justify-between items-center  bg-stake-700 backdrop-blur-md rounded-[20px] w-full mb-6 p-5'>
      ...
      <div className='flex flex-wrap justify-end items-baseline space-y-4 space-x-2 md:justify-start md:space-x-8 p-10'>
          ...
          <div>
            {
              asset.open ? (
                <button onClick={() => withdraw(asset.positionId)} className='py-1 px-4 text-md font-bold bg-stake-600/90 text-white rounded-md bg-gradient-to-br from-blue-600 to-primary-600 cursor-pointer border border-gray-500 drop-shadow-md hover:drop-shadow-2xl hover:text-lime-500'>Withdraw</button>
              ) : (
                <button className='py-1 px-4 text-md font-bold bg-gray-600/90 text-white rounded-md cursor-pointer border border-gray-500 drop-shadow-md' disabled>Closed</button>
              )
            }
          </div>
      </div>
